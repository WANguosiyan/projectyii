...
class LinkPager extends Widget
{
    ...
    protected function renderPageButtons()
    {
        ...
        if ($pageCount < 2 && $this->hideOnSinglePage) {
            return '<div class="col-md-5 col-sm-12"><div class="dataTables_info" id="sample_1_info" role="status" aria-live="polite">当前显示 '.$this->pagination->totalCount.' 条记录，总计 '.($this->pagination->totalCount).' 条</div></div>';
        }
        ...
        $html .= '<div class="col-md-5 col-sm-12"><div class="dataTables_info" id="sample_1_info" role="status" aria-live="polite">当前显示 '.(($this->pagination->getPageSize() * $this->pagination->getPage()) + 1).' 至 '.($page+1<$pageCount?$this->pagination->getPageSize() * ($this->pagination->getPage() + 1):$this->pagination->totalCount).' 条记录，总计 '.($this->pagination->totalCount).' 条</div></div>';
        $html .= '<div class="col-md-7 col-sm-12 dataTables_paginate paging_bootstrap_number">'.Html::tag('ul', implode("\n", $buttons), $this->options).'</div>';
        return $html;
    }
