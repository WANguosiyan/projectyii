validate(
    {
        ...
        'Props[p_order]':{'required':true,min:1}
    },
    {
        'Props[name]':{'required':'扩展属性名称不能为空'},
        'Props[p_order]':{'required':'排序不能为空','min':'排序至少为1'}
    }
);
...
$('.goods').on('click', function(){
    var obj = $(this);
    var props_id = obj.attr('data-id');
    var props_name = obj.parent().next().html();
    App.blockUI({
        target: 'body',
        animate: true
    });
    $.post(
        '?r=goods/props/goods',
        {'props_id':props_id,'_csrf':$('.request-csrf').val()},
        function (msg) {
            App.unblockUI('body');
            bootbox.dialog({
                message: msg,
                title: props_name+"-关联商品",
                buttons: {
                    main: {
                        label: "关闭",
                        className: "blue",
...
var TableDatatablesEditable = function () {
    ...
    var handleTable = function () {
        ...
        table.on('click', '.delete', function (e) {
            ...
            $.post(
                '?r=goods/propsvalue/delete&props_value_id='+value_id,
                {
        ...
        table.on('click', '.edit', function (e) {
            ...
            if (nEditing !== null && nEditing != nRow) {
                ...
                if (value_id == 0) {
                    ...
                    $.post(
                        '?r=goods/propsvalue/update&props_value_id='+value_id,
                        {
                            'name':jqInputs[0].value,
                            'alias':jqInputs[1].value,
                            'p_order':jqInputs[2].value,
